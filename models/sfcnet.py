...
from models.dpflosp import DPFLoSP
from models.unet2d import UNet2D
from models.unet3d import UNet3D
from models.sparse2dense import Sparse2Dense
from models.modules import ImageSemanticHead
...
class SFCNet(pl.LightningModule):
    def __init__(
        self,
        ...
        for scale_2d in self.scale_2ds:
            self.projects[str(scale_2d)] = DPFLoSP(
        ...
        self.imgsem_head = ImageSemanticHead(in_channels=64, num_classes=self.num_classes)
        # self.net_sparse2dense = Sparse2Dense()
        # log hyperparameters
    ...
    def forward(self, batch):
        ...
        imgsem_pred = self.imgsem_head(x_rgb["1_1"])
        x3ds = []
        map_vox_2s = []
        overlap_mask_2s = [] 
        map_vox_4s = []
        overlap_mask_4s = []
        # x_depth = self.net_sparse2dense(depth_img)
        x_depth = depth_img
        for i in range(bs):
            x3d = None
    ...
    def step(self, batch, step_type, metric):
        ...
        # if self.img_sem_loss:
        #     imgsem_pred = out_dict['imgsem_logit']
        #     imgsem_target = batch['semanticmask'].long()
        #     loss_fn = nn.CrossEntropyLoss(ignore_index=255)
        #     loss_imgsem = loss_fn(imgsem_pred, imgsem_target)
        #     loss += 0.5*loss_imgsem
        #     self.log(
        #         step_type + '/loss_imgsem',
        #         loss_imgsem.detach(),
        #         on_epoch=True,
        #         sync_dist=True,
        #     )
